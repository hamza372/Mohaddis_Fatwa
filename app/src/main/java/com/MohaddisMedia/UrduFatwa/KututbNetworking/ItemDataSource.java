...
public class ItemDataSource extends PageKeyedDataSource<Integer, FatwaDataModel> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull final LoadInitialCallback<Integer, FatwaDataModel> callback) {
        ...
        try {
            URL url = new URL("https://urdufatwa.com/api/questions/"+ Constants.BookNo +"/book?page="+FIRST_PAGE+"&perPage="+PAGE_SIZE);
            connection = (HttpURLConnection) url.openConnection();
            Log.d("tryURL","https://urdufatwa.com/api/questions/"+ Constants.BookNo +"/"+Constants.BookType+"?page="+FIRST_PAGE+"&perPage="+PAGE_SIZE);
            connection.connect();
            ...
            for(int i = 0;i<array.length();i++)
            {
                ...
                fatwaLink.setType(array.getJSONObject(i).getJSONObject("question").getString("type"));
    ...
    public void loadBefore(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, FatwaDataModel> callback) {
        ...
        try {
            URL url = new URL("https://urdufatwa.com/api/questions/"+ Constants.BookNo +"/book?page="+params.key+"&perPage="+PAGE_SIZE);
            connection = (HttpURLConnection) url.openConnection();
            connection.connect();
    ...
    public void loadAfter(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, FatwaDataModel> callback) {
        ...
        try {
            ...
            Log.d("tryPData",array.get(0).toString());
            Integer key = null;
            Log.d("tryLoadAfter",currentPage+"");
