...
public class ItemDataSourceImportant extends PageKeyedDataSource<Integer, FatwaDataModel> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull final LoadInitialCallback<Integer, FatwaDataModel> callback) {
        ...
        try {
            ...
            Log.d("tryNetURL","https://urdufatwa.com/api/all/"+ Constants.FatwaType +"?page="+FIRST_PAGE+"&perPage="+PAGE_SIZE);
            URL url = new URL("https://urdufatwa.com/api/all/"+ Constants.FatwaType +"?page="+FIRST_PAGE+"&perPage="+PAGE_SIZE);
           ...
           // JSONObject book = json.getJSONObject("book");
            ...
            for(int i = 0;i<array.length();i++)
            {
                ...
                fatwaLink.setFatwa_no(array.getJSONObject(i).getInt("id"));
    ...
    public void loadBefore(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, FatwaDataModel> callback) {
        ...
        try {
            URL url = new URL("https://urdufatwa.com/api/all/"+ Constants.FatwaType +"?page="+params.key+"&perPage="+PAGE_SIZE);
            connection = (HttpURLConnection) url.openConnection();
            connection.connect();
